...
Path_to_Table2_CSV  ="table2/bench_2m_2_comparepest.csv";
...
p  <- ggplot(performance_score,aes(fill =tool, y=mtf,x = tool ))+
  ...
  geom_text( # reliability scores
    data    = performance_score,
    mapping = aes(x = xreliab, y = y+10, label = repeatibility ),
    alpha = 1,
    size = 10
  )+
  geom_text( #reliability label
    data = performance_score,
    colour = "black",
    mapping = aes(x = xreliab+0.6+(N_Guidance_V-1), y = y+10, label = annotate_rel,fontface = "bold" ),
    size = 8
  ) 
